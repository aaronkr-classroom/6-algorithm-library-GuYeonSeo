...
#include <cctype> //isalnum(), issapce()
#include <iostream>
#include <string>
#include <vector>
...
iter url_begin(iter b, iter e) {
	...
	while ((i = search(i, e, sep.begin(), sep.end())) != e){
		//구분 기호가 현재 탐색범위의 처음 또는 마지막에 있는지 확인
		if (i != b && i + sep.size() != e) {
			...
			//구분 기호 앞뒤로 url의 일부에서 유효한 문작 하나라도 있는지 확인
			if (beg != i && !not_url_char(i[sep.size()])) {
				return beg;
			}
		}
		//발견한 구분 기호는 url의 일부가 아니므로 해당 구분 기호 이후를 표시하도록 i를 증가시킴
		i += sep.size();
	}
	return e;
}
...
vector<string>find_urls(const string& s) {
	...
	while (b != e) {
		...
		if (b != e) {
			...
			//인수로 전달받은 문자열에서 또 다른 url을 찾기 위해 b를 이동(증가)
			b = after;
		}
	}
	return urls;
